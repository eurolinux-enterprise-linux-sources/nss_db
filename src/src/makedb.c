...
static int process_input (FILE *input, const char *inname, DB *output,
...
main (int argc, char *argv[])
{
  ...
  if (do_undo)
    {
      status = db_open (input_name, DB_BTREE, DB_RDONLY, 0,
			NULL, NULL, &db_file);
      if (status)
	error (EXIT_FAILURE, 0, gettext ("cannot open database file `%s': %s"),
	       input_name, db_strerror (status));
...
static int
process_input (FILE *input, const char *inname, DB *output,
	       int to_lowercase, int be_quiet)
{
  char *line;
  size_t linelen;
  ...
  while (!feof (input))
      ...
      if (status != 0)
	{
	  if (status == DB_KEYEXIST)
	    ...
	    error (0, 0, gettext ("while writing database file: %s"),
		   db_strerror (status));
...
print_database (DB *db)
{
  ...
  if (status != 0)
    {
      error (0, 0, gettext ("while reading database: %s"),
	     db_strerror (status));
      return EXIT_FAILURE;
    }
